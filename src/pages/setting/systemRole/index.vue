  ...
  <div>
    ...
    <Card :bordered="false" dis-hover class="ivu-mt">
      ...
      >
        <Row type="flex" :gutter="24">
          ...
          <Col v-bind="grid">
            <FormItem label="角色名称：" label-for="role_name">
              <Input
                search
                enter-button
                placeholder="请输入角色名称"
      ...
      >
        ...
        <template slot-scope="{ row, index }" slot="rules">
          <div class="tex_ov">{{ row.rules }}</div>
        </template>
      </Table>
    ...
    >
      ...
      >
        ...
        <!-- <FormItem label="浏览范围：" prop="level">
          <RadioGroup v-model="formInline.level">
            <Radio :label="1">所有数据</Radio>
            <Radio :label="2">所属机构及下属机构所有数据</Radio>
            <Radio :label="3">个人数据</Radio>
          </RadioGroup>
        </FormItem> -->
        <FormItem label="权限：">
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        level: [
          { required: true, type: 'number', message: '请选择浏览权限', trigger: 'change' }
        ],
        status: [
