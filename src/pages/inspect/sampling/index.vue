  ...
  <div>
    ...
    <Card :bordered="false"
        ...
        <Row type="flex">
          ...
          <Upload :action="fileurlL"
            ...
            <Button style="margin-left: 15px"
                    type="success"
                    class="bnt mr15">批量导入</Button>
          </Upload>
          <Button type="success"
                  class="bnt mr15"
                  @click="exports">{{downloadstr}}</Button>
        </Row>
      </Form>
      ...
      <Modal v-model="excelDatamodals"
        ...
        <List border
              size="small">
          <ListItem v-for="(item, index) in excelData"
                    :key="index">
            <Alert type="success"
                   v-if="item.error == 0"
                   style="width: 100%">
...
export default {
  ...
  methods: {
    ...
    edit (row) {
      this.$router.push({
        path: `/admin/inspect/samplingDetails/${row.id}`
      })
