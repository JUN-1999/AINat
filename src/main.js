...
import './assets/iconfontQuanping/iconfont.css'
...
//刷新加载菜单
// import { adminInfoAuth } from '@/api/setting'
...
new Vue({
    ...
    async created() {
        ...
        // adminInfoAuth().then(res=>{
        //   var menus = res.data.data.menus;
        //   console.log('重新请求菜单');
        //   this.$forceUpdate();
        //   this.$store.commit("admin/menus/getmenusNav", menus);
        // })
    },
    watch: {
        ...
        '$route' (to, from) {
            ...
            if (Setting.dynamicSiderMenu) {
                ...
                if (!menus.length) {
                    if (path !== '/ainat/login' && path !== '/ainat/screen' && path!=='/ainat/home') {
                        this.$router.replace('/ainat/login');
                    }
                    return
                }
                ...
                if (headerName !== null) {
                    ...
                    if (Setting.layout.headerMenu) {
                        ...
                        this.$store.commit('ainat/menu/setHeader', headerSider);
                        // 指定当前侧边栏隶属顶部菜单名称。如果你没有使用顶部菜单，则设置为默认的（一般为 home）名称即可
                        this.$store.commit('ainat/menu/setHeaderName', headerName);
                        // 获取侧边栏菜单
                        const filterMenuSider = getMenuSider(menuSider, headerName);
                        // 指定当前显示的侧边菜单
                        this.$store.commit('ainat/menu/setSider', filterMenuSider);
                    } else {
                        // 指定当前侧边栏隶属顶部菜单名称。如果你没有使用顶部菜单，则设置为默认的（一般为 home）名称即可
                        this.$store.commit('ainat/menu/setHeaderName', 'home');
                        // 指定当前显示的侧边菜单
                        this.$store.commit('ainat/menu/setSider', menuSider);
                    }
                    // 指定当前菜单，即高亮项
                    this.$store.commit('ainat/menu/setActivePath', path);
                    // 找到其所有父菜单 path 进行展开
                    // const openNames = getSiderSubmenu(path, menuSider);
                    const openNames = getSiderSubmenu(to, menuSider);
                    this.$store.commit('ainat/menu/setOpenNames', openNames);
                } else {
