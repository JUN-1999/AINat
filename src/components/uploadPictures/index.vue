              ...
              <div class="acea-row">
                ...
                >
                  ...
                  >
                    ...
                    />
                    <div class="nameStyle" v-show="item.isEdit">
                      {{ item.real_name }}
                    </div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    renderContent(h, { root, node, data }) {
      ...
      //     )
      // }
      if ((data.pid === 0 && data.nodeKey !== 0) || data.pid !== 0) {
        operate.push(
          h(
            "div",
            {
    ...
    editName(index) {
      this.pictrueList[index].isEdit = !this.pictrueList[index].isEdit;
      let name = this.pictrueList[index].real_name + index;
      this.$nextTick((e) => {
      ...
      fileListApi(this.fileData)
        .then(async (res) => {
          res.data.list.forEach((el) => {
            ...
            el.num = 0;
            el.editName =
              el.real_name.substr(0, 2) + "..." + el.real_name.substr(-6, 6);
          });
    ...
    // 新建分类表单
    getFrom() {
      this.$modalForm(createApi({ id: this.treeId })).then(() =>
        this.getList()
      );
    },
    // 上传之前
