...
service.interceptors.request.use(
  async config => {
    if (config.kefu) {
      let baseUrl = Setting.apiBaseURL.replace(/ainatapi/, "kefuapi")
    ...
    config.headers['timestamp'] = timestamp.substring(0, 10)
    config.headers['Authori-zation-screen'] = 'Bearer ' + md5('ldrk_adminapi_' + timestamp.substring(0, 10))
    // 开启loading
...
service.interceptors.response.use(
  response => {
    ...
    switch (code) {
      ...
      case 410002:
        console.log('aaaaaa');
        window.router.replace('/ainat/login');
        break;
      case 410003:
